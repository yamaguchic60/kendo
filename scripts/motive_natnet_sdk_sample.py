# りんたろうから送って貰った、pythonで座標を取得するためのスクリプトです。
...
def receive_new_frame(data_dict):
    """
    Motiveから新しいフレームデータを受信するたびに呼び出されるコールバック関数。
    剣先のリジッドボディの座標を取得して表示する。
    """
    rigid_bodies = data_dict.get("rigidBodies")
    if rigid_bodies:
        for rigid_body in rigid_bodies:
            if rigid_body.get("name") == "Object":  # Motiveで設定した剣先のリジッドボディ名 (画像から"Object"と推測)
                # 剣先の座標を取得
                x = rigid_body.get("position")[0]
                y = rigid_body.get("position")[1]
                z = rigid_body.get("position")[2]

                # 座標を表示
                print(f"SwordTip (Object) Coordinates: x={x:.3f}, y={y:.3f}, z={z:.3f}")  # 小数点以下3桁で表示

                # (オプション) 座標データをファイルに保存するなどの処理をここに追加

                break  # 剣先のリジッドボディを見つけたらループを抜ける
...
if __name__ == "__main__":
    ...
    optionsDict = {}
    optionsDict["clientAddress"] = "172.27.246.149" #pythonコード側のPC
    optionsDict["serverAddress"] = "10.103.81.131" #motive入っている側のPC
